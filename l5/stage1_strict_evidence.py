...
subprocess.run(["python", "l5/stage2_round1_repair.py"], check=True)
stage2_review = ROOT / "stage2_visual_review_round1"
subprocess.run(["rm", "-rf", str(stage2_review)], check=True)
(stage2_review / "new_pdf").mkdir(parents=True, exist_ok=True)
round1_ppt = ROOT / "ECE340_L5_S18_Posted_中文忠实重建_第二阶段视觉返修ROUND1_第8_10_11_14_16_17_19_22_24页.pptx"
subprocess.run([
    "libreoffice", "--headless", "--convert-to", "pdf", "--outdir", str(stage2_review / "new_pdf"),
    str(round1_ppt)
], check=True)
subprocess.run(["python", "l5/stage2_round1_evidence.py"], check=True)
subprocess.run([
    "git", "add", "--",
    str(round1_ppt),
    "l5/BUILD_REPORT_STAGE2_ROUND1.md",
    "l5/stage2_round1_assets",
    "l5/stage2_visual_review_round1"
], check=True)
print("stage2 round1 outputs staged for commit")
